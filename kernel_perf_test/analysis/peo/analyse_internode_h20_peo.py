...
_KERNEL_FAKE_BALANCE_NAME = "void rtp_llm::fakeBalanceExpertKernel"
_KERNEL_DISPATCH_NAME = "void deep_ep::internode_ll::dispatch_opt2_hidden6144"
_KERNEL_COMBINE_NAME = "void deep_ep::internode_ll::combine_opt2_hidden6144"
_KERNEL_RMSNORM_NAME = "void flashinfer::norm::FusedAddRMSNormKernel"
...
def _extract_moe_durations_us_from_kernels(kernels: list[KernelEvent]) -> list[float]:
    ...
    while i + 1 < n:
        ...
        if not (_KERNEL_FAKE_BALANCE_NAME in k0.name and _KERNEL_DISPATCH_NAME in k1.name):
            i += 1
            continue
        ...
        while j + 1 < n:
            c0 = kernels[j]
            c1 = kernels[j + 1]
            if _KERNEL_COMBINE_NAME in c0.name and _KERNEL_RMSNORM_NAME in c1.name:
...
def main() -> None:
    ...
    batch_sizes = sorted(
        set(discover_batch_sizes_from_benchmark_dir(normal_dir)) | set(discover_batch_sizes_from_benchmark_dir(peo_dir))
    )
    if not batch_sizes:
    ...
    for b in batch_sizes:
        ...
        for o in [1, 2, 3, 4]:
            ...
            if r:
                line.append(f"OVERLAP_{o}={r.latency_us:.3f} (rounds={r.num_peo_rounds}, sm={r.deep_ep_num_sm})")
            else:
